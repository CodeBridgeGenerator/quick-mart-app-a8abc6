...
const ItemCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.supplierId)) {
                error["supplierId"] = `SupplierId field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,description: _entity?.description,price: _entity?.price,stockQty: _entity?.stockQty,category: _entity?.category,supplierId: _entity?.supplierId,supermarketId: _entity?.supermarketId,itemId: _entity?.itemId,
