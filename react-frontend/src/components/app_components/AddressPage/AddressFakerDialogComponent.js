...
import addressFakerFactory from "./addressFakerFactory";
...
const AddressFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = addressFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("address").create(elem));
...
export default connect(mapState, mapDispatch)(AddressFakerDialogComponent);
