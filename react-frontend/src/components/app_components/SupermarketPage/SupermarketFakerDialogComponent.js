...
import supermarketFakerFactory from "./supermarketFakerFactory";
...
const SupermarketFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = supermarketFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("supermarket").create(elem));
...
export default connect(mapState, mapDispatch)(SupermarketFakerDialogComponent);
