...
import orderItemFakerFactory from "./orderItemFakerFactory";
...
const OrderItemFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = orderItemFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("orderItem").create(elem));
...
export default connect(mapState, mapDispatch)(OrderItemFakerDialogComponent);
